    ...
    def __init__(self):
        ...
        if self.ENVIRONMENT == "production":
            ...
            if not self.GCP_PROJECT_ID:
                raise ValueError(
                    "GCP_PROJECT_ID environment variable not set in production."
                )
            ...
            self.ODOO_PROD_USERNAME = self._fetch_gcp_secret(
                "ODOO_PROD_USERNAME", gcp_client
            )
            self.ODOO_PROD_PASSWORD = self._fetch_gcp_secret(
                "ODOO_PROD_PASSWORD", gcp_client
            )
            ...
            self.ODOO_TEST_USERNAME = self._fetch_gcp_secret(
                "ODOO_TEST_USERNAME", gcp_client
            )
            self.ODOO_TEST_PASSWORD = self._fetch_gcp_secret(
                "ODOO_TEST_PASSWORD", gcp_client
            )

            # Langsmith
            self.LANGSMITH_API_KEY = self._fetch_gcp_secret(
                "LANGSMITH_API_KEY", gcp_client
            )
            self.LANGSMITH_PROJECT = self._fetch_gcp_secret(
                "LANGSMITH_PROJECT", gcp_client
            )
            ...
            self.PRODUCT_DB_PASSWORD = self._fetch_gcp_secret(
                "PRODUCT_DB_PASSWORD", gcp_client
            )
            ...
            self.EMILIA_DB_PASSWORD = self._fetch_gcp_secret(
            ...
            self.EMILIA_DOCS_SUMMARY_ID = self._fetch_gcp_secret(
                "EMILIA_DOCS_SUMMARY_ID", gcp_client
            )
            ...
            if self.ENVIRONMENT == "local_container":
                print(
                    "✅ Running in 'local_container' mode. Loading settings from .env file (via docker-compose)."
                )
            else:  # 'local_machine'
                print(
                    "✅ Running in 'local_machine' mode. Loading settings from .env file directly."
                )
            ...
            raise ValueError(
                f"Unknown ENVIRONMENT: '{self.ENVIRONMENT}'. Must be one of 'production', 'local_container', or 'local_machine'."
            )
